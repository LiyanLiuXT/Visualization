...
function drawGrids(){
    ...
    let aScale = d3.scale.linear() //透明度比例尺
        .domain([1, 4])
        .range([0.4, 1]);
    let foScale = d3.scale.linear() //fill透明度比例尺
        .domain([1, 4])
        .range([0.2, 0.5]);
    let lScale = d3.scale.linear() //边框粗细比例尺
        .domain([1, 4])
        .range([1, 4]);
    let hour = parseInt(time[Handle].slice(0,2));
    console.log("hour",hour);
    console.log("Handle",Handle);
    console.log("time",time);
    if (hour<10 && hour>=0){
        searchHour = "0".concat(hour.toString());
        }
    else{
        searchHour = hour.toString();
    }
    //origin
    ...
    //get keyGrids first five number put into showDict and the extendDict which depends on zoom size(>20<)
    $.getJSON("./data/keyGrids.json",function(data) {
        let count = 0;
        showDict = {};
        {
            let extendDict = new Array();
            let opacity = 1;
            let fillOpacity = 1;
            let zoom = map.getZoom();
            $.each(data, function (index, e) {
                let time = parseInt(e.slice(6, 8));
                if (time_one <= time && time < time_two) {
                    if (count < 20) {

                        let gridIndex = e.slice(0, 5);
                        ...
                        if (extendDict[gridIndex] != null) {
                            ...
                            if (zoom > 12) {
                                if (extendDict[gridIndex] == null) {
                                    for (let k = 99; k < 102; k++) {
                                        neiIndex = neighbor(gridIndex, k);
                                        extendDict[neiIndex] = gridIndex;//TODO  Is this line need to in the for cycle?
                                    }
                                }
            ...
            //将需要显示的grid显示在地图上
            let gridCount = 0;
            for (let key in showDict) {
                let grid_index = parseInt(key);
                let i = parseInt(grid_index / blockCount);
                let j = grid_index % blockCount;
                let mergerNum = showDict[key].num;
                //var larger=0.5*(1-Math.sqrt(mergerNum)/mergerNum);
                //console.log(zoom+","+key+","+mergerNum)
                let larger = 0.2 * Math.log(mergerNum, 1.5);
                let bound = [[bottom + (i - larger) * heightDistance, left + (j - larger) * wideDistance], [bottom + (i + 1 + larger) * heightDistance, left + (j + 1 + larger) * wideDistance]];
                color = '#FF216C'
                /*if(zoom>12){
                  opacity=0.4;
                }
...
// add num to grid and fill with 0 in number to 5 digits
// ex neighbor(4,1) -> 00005
...
//fill the number digit with '0' to count digits\
//ex  string_transfer(3,2) -> 03
function string_transfer(str,count){
    str=str.toString(10);//转为10进制、16进制
    if(str.length<count){
        let x=count-str.length;
        let addStr='';
        for(let i=0;i<x;i++){
            addStr=addStr+'0';
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//画连线
    let svgLines = g.attr("class", "lines")
        .selectAll("g")
        .data(links)
        .enter().append("g")
        .attr("class", "line");

    let defs = svgLines.append("defs");
...
//var c=1;
    for (let i=0; i<links.length;i++){
        let start=links[i]["source"]["gridIndex"];
        let end=links[i]["target"]["gridIndex"];
        let hour=links[i]["source"]["starttime"];
        ////////////////////////////////////////////////////////////////////////////////////////
        let start_grid=string_transfer(start,5);
        let end_grid=string_transfer(end,5);
        let start_grid_num=showDict[start_grid].num;
        let end_grid_num=showDict[end_grid].num;
        let lineWidth=0;
        grid_grid_device_dict={};
        // console.log(start_grid_num);
        for(let m=0; m<start_grid_num;m++){
            for(let n=0;n<end_grid_num;n++){
                let sgrid=parseInt(showDict[start_grid].subgrids[m]);
                let egrid=parseInt(showDict[end_grid].subgrids[n]);
                let grid_grid=string_transfer(sgrid,4)+"_"+string_transfer(egrid,4)+"_"+string_transfer(hour,2);
                // console.log("sgrid",sgrid);
                // console.log("egrid",egrid);
                // console.log("grid_grid",grid_grid);
                // console.log("showDict", showDict);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        let gridInfo=string_transfer(start,4)+"_"+string_transfer(end,4)+"_"+string_transfer(hour,2);
        //console.log(gridInfo+', '+linkNum[gridInfo]+', '+start_grid+', '+showDict[start_grid].num)
        let rScale = d3.scale.linear()
            .domain([0, 13])
            .range([0.5, 0.75]);

        let oScale = d3.scale.linear()
            .domain([0, 13])
            .range([0.1, 0.6]);
    ...
    for (var i=0; i<links.length;i++){
        ...
        for(var m=0; m<start_grid_num;m++){
            for(var n=0;n<end_grid_num;n++){
                ...
                ///////////////////////////////////////////////////////////////////////
                // console.log("gridgridDeviceTopic",gridgridDeviceTopic);
                if(gridgridDeviceTopic[grid_grid]!=null){
                    //console.log(gridgridDeviceTopic[grid_grid]);
                    for (var p=0;p<gridgridDeviceTopic[grid_grid].length;p++){
                        var device=gridgridDeviceTopic[grid_grid][p].id;
        ...
        // console.log("grid_grid_device_dict",grid_grid_device_dict);
        for(key in grid_grid_device_dict){
            lineWidth=lineWidth+1;
        }
        //////////////////////////////////////////////////////////////////////////////////////
...
//save gridgridDeviceTopicInfo into memory
function readGridgridDeviceTopicFile(hour){
    gridgridDeviceTopic={};
    console.log("hour",hour);
    $.ajax({
        url: "./data/".concat(hour).concat("/gridgridDeviceTopicInfo.json"),
        dataType: 'json',
